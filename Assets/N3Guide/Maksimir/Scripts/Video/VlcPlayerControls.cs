...
using LibVLCSharp;
using Novena.Components.AvProVideoPlayer.Controls;
using DG.Tweening;
...
public class VlcPlayerControls : MonoBehaviour {
	...
	private void Awake()
	{
		...
		VideoDetailsViewController.OnMediaPlayerInstantiated += StartRoutine;
		VlcPlayer.OnVideoOpened += OnVideoOpened;
	}
	...
	{//VLC Event Handlers
		...
		playPauseButton.onClick.AddListener(() => {
			if (vlcPlayer.mediaPlayer == null) return;
	...
	void Update()
	{
		if (UnityEngine.Time.timeSinceLevelLoad < 2.2f) return;
	...
	{
		if (UserInteraction.IsUserInputThisFrame() || /*!CanHideControls() ||*/ !_isPlaying)
		{
		...
		else
		{
			...
			if (UserInteraction.InactiveTime >= 1.5f)
			{
				ShowControls(false);
			}
			else
	...
	{
		controlsGroup.DOFade(isShow ? 1 : 0, 0.1f);
		controlsGroup.blocksRaycasts = isShow;
	}
	...
	{
		playIcon.gameObject.SetActive(!playing);
		pauseIcon.gameObject.SetActive(playing);
	}
